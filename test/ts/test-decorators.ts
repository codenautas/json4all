...
for(var base of ['assert', 'discrepances']){ assert.setBase(base); describe(base, ()=>{
...
describe("---- TYPESCRIPT Referenceable objects", ()=>{
    ...
    it("deserializes a reference", ()=>{
        var two = JSON4all.parse(`{"$ref":[["TheCollection"],"two"],"$special":"Two","$value":{"name":"Other","more":1}}`) as any;
        assert.ok(two instanceof Two);
